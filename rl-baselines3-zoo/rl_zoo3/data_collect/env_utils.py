...
def make_data_collect_vec_env(
    ...
    def make_env(rank: int) -> Callable[[], gym.Env]:
        def _init() -> gym.Env:
            ...
            # Wrap the env in a DataCollectWrapper to collect gameplay data
            # with (image, action) pairs
            data_save_path = os.path.join(data_collect_dir, str(rank)) if data_collect_dir is not None else None
            # Create the data save folder if needed
            if data_save_path is not None and data_collect_dir is not None:
                os.makedirs(data_collect_dir, exist_ok=True)
            env = DataCollectWrapper(env, output_dir=data_save_path)
